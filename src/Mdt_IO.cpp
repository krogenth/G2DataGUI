...
void readMdt(std::vector<MdtStruct>& mdt, std::vector<njcmStruct>& models, std::string filepath) {
	...
	for (const auto& p : std::filesystem::directory_iterator(filepath)) {
		...
		if (std::filesystem::is_directory(p)) {
			...
			for (const auto& q : std::filesystem::directory_iterator(p)) {
				...
				mdt.back().header = readRaw<mdtHeader>(input);

				//	read in map entries
				mdt.back().mapEntries.resize(mdt.back().header.numMapEntries);
				input.seekg(mdt.back().header.offsetMapEntries, std::ios::beg);

				for (size_t i = 0; i < mdt.back().mapEntries.size(); i++)
					mdt.back().mapEntries[i] = readRaw<MapEntriesStruct>(input);


				//	read in instances
				mdt.back().instances.resize(mdt.back().header.numInstances);
				input.seekg(mdt.back().header.offsetInstances, std::ios::beg);

				for (size_t i = 0; i < mdt.back().instances.size(); i++)
					mdt.back().instances[i] = readRaw<InstancesStruct>(input);


				//	read in HTA
				mdt.back().HTA.resize(mdt.back().header.numHTA);
				input.seekg(mdt.back().header.offsetHTA, std::ios::beg);

				for (size_t i = 0; i < mdt.back().HTA.size(); i++)
					mdt.back().HTA[i] = readRaw<HTAStruct>(input);


				//	read in enemy positions
				mdt.back().enemyPositions.resize(mdt.back().header.numEnemyPos);
				input.seekg(mdt.back().header.offsetEnemyPos, std::ios::beg);

				for (size_t i = 0; i < mdt.back().enemyPositions.size(); i++)
					mdt.back().enemyPositions[i] = readRaw<EnemyPositionStruct>(input);


				//	read in enemy groups
				mdt.back().enemyGroups.resize(mdt.back().header.numEnemyGroup);
				input.seekg(mdt.back().header.offsetEnemyGroup, std::ios::beg);

				for (size_t i = 0; i < mdt.back().enemyGroups.size(); i++)
					mdt.back().enemyGroups[i] = readRaw<EnemyGroupStruct>(input);


				//	read in MOS
				mdt.back().MOS.resize(mdt.back().header.numMOS);
				input.seekg(mdt.back().header.offsetMOS, std::ios::beg);

				for (size_t i = 0; i < mdt.back().MOS.size(); i++)
					mdt.back().MOS[i] = readRaw<MOSStruct>(input);


				//	read in icons
				mdt.back().icons.resize(mdt.back().header.numIcons);
				input.seekg(mdt.back().header.offsetIcons, std::ios::beg);

				for (size_t i = 0; i < mdt.back().icons.size(); i++)
					mdt.back().icons[i] = readRaw<IconStruct>(input);


				//	read in shop
				if (mdt.back().header.offsetShop) {

					mdt.back().shop.resize(1);
					input.seekg(mdt.back().header.offsetShop, std::ios::beg);

					mdt.back().shop[0] = readRaw<ShopStruct>(input);

					for (ImU16 i = 0; i < 12; i++) {

						if(mdt.back().shop[0].weapons[i].item)
							mdt.back().shop[0].weapons[i].item -= 0x0800;

						if (mdt.back().shop[0].armors[i].item)
							mdt.back().shop[0].armors[i].item -= 0x0800;

						if (mdt.back().shop[0].jewelry[i].item)
							mdt.back().shop[0].jewelry[i].item -= 0x0800;

						if (mdt.back().shop[0].items[i].item)
							mdt.back().shop[0].items[i].item -= 0x0800;

						if (mdt.back().shop[0].regionals[i].item)
							mdt.back().shop[0].regionals[i].item -= 0x0800;
				...
				*/
				...
				mdt.back().filename = q.path().u8string();
				mdt.back().filenameChr = q.path().u8string().substr(0, q.path().u8string().length() - 4) + ".chr";
