...
void SpecialsClass::draw() {
	...
	if (ImGui::BeginCombo("Move", this->_moves[this->_specials.at(this->_specialIndex).moves[this->_moveIndex].moveOffset].name)) {
		...
		for (size_t i = 0; i < this->_numMoves; i++) {
			...
			if (ImGui::Selectable(this->_moves[i].name, is_selected))
				this->_specials.at(this->_specialIndex).moves[this->_moveIndex].moveOffset = i;
			if (is_selected)
				ImGui::SetItemDefaultFocus();
			ImGui::PopID();
