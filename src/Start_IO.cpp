...
void drawPC(std::vector<StartStatsStruct>& stats, bool* canClose, char** itemIDs, const size_t& numItems) {
	...
	ImGui::InputShort("T_REC", &stats[statID].TREC);													if (ImGui::IsItemHovered()) ImGui::SetTooltip("Time to recover from status effects(lower is better).");
	ImGui::InputShort("T_DMG", &stats[statID].TDMG);													if (ImGui::IsItemHovered()) ImGui::SetTooltip("Time to recover from being hit(lower is better)?");

	ImGui::InputUShort("Unknown #14", &stats[statID].unknown14);

	ImGui::InputShort("T_HEAL", &stats[statID].THEAL);
